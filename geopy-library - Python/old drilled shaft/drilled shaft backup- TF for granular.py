...
def internal_friction_to_contact_friction(phi): #assumes concrete 
  return 0.75 * phi
...
#Every layer must be divided evenly by the sublayer thickness
  ...
  for stratum in raw_profile: #for each stratum given by user
    ...
    for i in range(int(num_sublayers)): #for each layer, create sublayers that add up to full height of layer
      soil_profile.append([sublayer_thickness, stratum[1], stratum[2], stratum[3]]) 
  return soil_profile
...
#start counting side friction for either granular or cohesive layer
  for i in range(int(ignored_depth / layer_thickness), len(soil_profile)):
    if soil_profile[i][2] == True:
      side_friction = side_friction_granular(soil_profile[i][0], soil_profile[i][3], effective_stress_profile[i][1], is_compressive, diameter)
    else:
      side_friction = side_friction_cohesive(soil_profile[i][0], soil_profile[i][3], diameter)
...
def calculate_ultimate_toe_bearing(soil_profile, effective_stress_profile, embedment_depth, layer_thickness, diameter):
  if soil_profile[int(embedment_depth / layer_thickness)][2] == True: #if layer BENEATH embedment is granular
    return end_bearing_granular(effective_stress_profile[int(embedment_depth / layer_thickness) - 1][1], soil_profile[int(embedment_depth / layer_thickness)][3], diameter) #calc end bearing based on stress AT embedment
  else:
    return end_bearing_cohesive(soil_profile[int(embedment_depth / layer_thickness)][3], embedment_depth, diameter)

def calculate_allowable_load(ultimate_side_friction, ultimate_toe_bearing, safety_factor):
...
def analyze_pile(diameter, embedment_depth, is_compressive, safety_factor):
  ...
  ultimate_toe_bearing = calculate_ultimate_toe_bearing(divided_soil_profile, effective_stress_profile, embedment_depth, layer_thickness, diameter)
  allowable_load = pounds_to_kips(calculate_allowable_load(ultimate_side_friction, ultimate_toe_bearing, safety_factor))
  print(allowable_load)
...
layer_thickness = 1
depth_to_water_table = 100
ignored_depth = 3
divided_soil_profile = create_soil_profile(input_soil_profile, layer_thickness)

analyze_pile(8, 6, True, 2)
analyze_pile(8, 10, True, 2)
analyze_pile(8, 20, True, 2)
analyze_pile(8, 30, True, 2)
analyze_pile(8, 40, True, 2)
analyze_pile(8, 50, True, 2)
